...
def fetch_Run_IDs_with_pysradb(sra_id):
    ...
    if pysra_command_worked:
        ...
        for i, row in enumerate(rows):
            ...
            fields = row.split("\t")
            try:
                run_idx = row.index("run_accession")
                total_size_idx = row.index("total_size")
            except ValueError as e: ## if either the number of fields is wrong, or if the typecast fails (say if total_size == "<NA>"),  then skip this run_accession.
                continue
            ...
            run_accession = fields[run_idx]
            total_size_idx = fields[total_size_idx]
            int_total_size = int(total_size)
            if int_total_size > 0 and "WGS" in row and "ILLUMINA" in row and run_accession != "nan":
